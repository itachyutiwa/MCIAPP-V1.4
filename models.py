...
def to_camel_case(column_name):
    ...
    if column_name.lower() in ["date_reception", "date reception"]:
...
# --------Conformez toutes les variables benef, coti et conso à celles de test.py
    ...
    def production(self, file_benef, file_cotisation, file_conso):
        ...
        return {
            "production": f"{result_1} : bénéficiaire(s) ayant eu des prestations sans cotiser."
        }
    ...
    def affiliation_avnants(self, file_benef, file_cotisation, file_conso):
        ...
        return {
            "polices": f"{result_1} : bénéficiaire(s) sans cotisation."
        }
    ...
    def recouvrement(self, file_benef, file_cotisation, file_conso):
        ...
        return {
            "recouv":  f"{list(set(result_2))} : bénéficiaire(s) sans cotisation ayant consommé(s), {list(set(result_3))} : prestation(s) sans bénéficiaire(s) ni cotisations( cas grave)."
        }
...
class Prestation:
    ...
    def conso_autorisees(self, file_benef, file_cotisation, file_conso):
        ...
        result_3 = [id for id in df3 if id not in df1 and id not in df2]
        return {
            "conso_auto": f"{list(set(result_1))} : bénéficiaire(s) sans cotisation ayant consommé(s), {list(set(result_3))} : prestation(s) sans bénéficiaire(s) ni cotisations( cas grave)."
        }
    ...
    def periode_couverture(self, file_benef, file_cotisation, file_conso, date_fin):
        ...
        filtrage = set(list(
            consommation[consommation['numPolice'].isin(cotisation_filtre)]['numPolice']))

        return {
            "periode_couv": f"{list(filtrage)} : prestations à une date d'émission après la période de contrôles."
        }
    ...
    def condition_age(self, file_benef, file_cotisation, file_conso, date_fin="31/12/2022"):
        ...
        return {
            "cndt_age": f"{list(set(ENFANT_SUP_25_CONSO))} : prestations enfants de plus de 25 ans, {list(set(ADULTES_SUP_60_CONSO))} : prestations adultes de plus de 60 ans."
        }
    ...
    def double_conso(self, file_benef, file_cotisation, file_conso):
        ...
        return {
            "db_conso": f"{list(set(dble_conso))} : police(s) avec double prestations."
        }
    ...
    def delai_contractuel(self, file_benef, file_cotisation, file_conso):
        ...
        return {
            "delai_cnt": f"{liste} : prestations avec date de soins après plus de tois (03) mois de la date d'émission , {liste_} : prestations sans date émission"
        }
    ...
    def depense_imortantes(self, file_benef, file_cotisation, file_conso, conso_ctx):
        ...
        return {
            "dp_mpt": f"{n_premiers_NUM_POLICE} : quelques prestations importantes."
        }
